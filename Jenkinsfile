node {
  ...
  stage("Tests and Deployment") {
    ...
    stage("Deployment") {
      sh "chmod +x ./gradlew"
      sh 'nohup ./gradlew bootRun -Dserver.port=8081 &'
      sh 'echo #!asdf2580 | sudo docker login -u muratbek7@gmail.com --password-stdin'
      sh 'sudo docker build -t muratbek7/restapidemo:1.0 .'
    }
    stage('Push Docker Image') {
        script {
            docker.withRegistry('https://index.docker.io/v1/', dockerHubCredentialsId) {
                def image = docker.image("muratbek7/restapidemo:1.0")
                image.push()
                image.push('1.0')
            }
        }
